...
class ArithmeBricksGame(Widget):
    ...
    def provide_bricks(self):
        ...
        for symbol in self.symbol_generator(limits):
            self.add_new_brick(symbol,i)
            
            #inputText = inputText + ' ' + str(i)+ ' ' + symbol 
            i=i+1

        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) #from chat_client
        self.socket.connect((SERVER_IP, 22222))
            #self.input = self.socket.makefile('rb', 0)
        self.output = self.socket.makefile('wb', 0)
        #print ("len=%d" % len (Nodes))
        inputText = "<create> "
        for j in Nodes.keys():
            inputText = inputText + str(j) + ' ' + Nodes[j][0] + ' ' + str(int(Nodes[j][1])) + ' ' + str(int(Nodes[j][1])) +' '
        inputText += '\n'
        print ("write:%s"% inputText)
    ...
    def add_new_brick(self, symbol, id):
        ...
        #print ('brickid: %s, symbol: %s, x:%d, y:%d' % (brick.id, symbol, brick.pos[0], target_pos[1] ))
        Nodes[id]=[symbol,target_pos[0],target_pos[1]]
        #print (Nodes[id])
    
    def show_brick(self, id,symbol,  x,y):
        target_pos = self.set_pos(x,y)
    ...
    def popup_view(self):    
        ...
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) #from chat_client
        self.socket.connect(("localhost", 22222))
        self.input = self.socket.makefile('rb', 0)
        self.output = self.socket.makefile('wb', 0)
        self.output.write("<receiver>")
        
        msg = self.input.readline()
        print("receive: %s" % msg)
        #received_msg = msg.decode()
        
        #split()
        #print ("len:" + len(msg.split())
        range1=len(msg.split())
        for i in xrange(1,range1,4):
            id1=msg.split()[i]
            
            symbol1=msg.split()[i+1]
           
            x1 = msg.split()[i+2]
            
            y1 = msg.split()[i+3]
            
            self.show_brick(int(id1),str(symbol1),int(x1),int(y1))
            print ("id:" + id1 +' '+ symbol1 +' '+ x1 +' '+ y1)
...
class Brick(DragBehavior, Label):
    ...
    def on_touch_up(self, touch):
        if self.state != 'final' and super(Brick, self).on_touch_up(touch):
            ...
            print("id: %s, symbol: %s, x:%d, y:%d" % (self.id, self.text,self.pos[0],self.pos[1] ))
            intputText  = []
            inputText = self.id + ' ' + self.text +' ' + str(self.pos)+ ' ' +str(self.pos[1])
